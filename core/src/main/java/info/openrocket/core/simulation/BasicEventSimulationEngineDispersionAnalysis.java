...
public class BasicEventSimulationEngineDispersionAnalysis implements SimulationEngine {
	...
	private static final Logger log = LoggerFactory.getLogger(BasicEventSimulationEngineDispersionAnalysis.class);
	...
	private double [] pressureIndexMap; //used for determining which index corresponds to the pressure
	...
	public void setAltToWind(double [] altToWind, double[] altToWindDirection,double [] pressureIndexMap ){
		this.altToWind = altToWind;
		this.altToWindDirection = altToWindDirection;
		this.pressureIndexMap = pressureIndexMap;

		variableWindBoolean = true;
	}

	public void setThrustCurve(double [] thrust, double [] time){
		this.thrust = thrust;
		this.time = time;
	}
	...
	public void updateWind(){
		...
		double altitude = currentStatus.getRocketWorldPosition().getAltitude();
		double curr_pressure = currentStatus.getSimulationConditions().getAtmosphericModel().getConditions(altitude).getPressure()/100; //converts to hPA

		double wind = 0.001;
		double windDirection = 0.001;
		//Finds the index in alt wind and wind direction using pressure
		int prevIndex = 0;

		//If below minimium, presure, no point in looping
		if (curr_pressure < pressureIndexMap[pressureIndexMap.length-1]){
//			wind = altToWind[altToWind.length-1];
//			windDirection = altToWindDirection[altToWindDirection.length-1];
			wind = 0;
			windDirection = 0;
		}
		else if (curr_pressure > pressureIndexMap[0]){
			wind = 0;
//			wind = altToWind[0];
//			windDirection = altToWindDirection[0];
		}
		else {
			for (int i = 1; i <pressureIndexMap.length; i++) {
				//assuming pressure is between two values
				if ((pressureIndexMap[prevIndex] >= curr_pressure) && (curr_pressure >= pressureIndexMap[i])) {
					wind = linearModel(pressureIndexMap[prevIndex], altToWind[prevIndex], pressureIndexMap[prevIndex + 1], altToWind[prevIndex + 1], curr_pressure);
					windDirection = linearModel(pressureIndexMap[prevIndex], altToWindDirection[prevIndex], pressureIndexMap[prevIndex + 1], altToWindDirection[prevIndex + 1], curr_pressure);
					break; //Ends for loop, after finding correct indicies
				}
				prevIndex = i;
...
//
	...
	/**
	 * Generates a linear interpolation model for wind and direction
	 * @param x1
	 * @param y1
	 * @param x2
	 * @param y2
	 */
	public double linearModel(double x1, double y1, double x2, double y2, double x3){
	...
	private FlightDataBranch   simulateLoop() throws SimulationException {
		...
		try {
			...
			while (handleEvents()) {
				...
				//TODO: New Eric Code, used for changing Thrust for twr for Heavybulb 2 sizing
//				ArrayList<MotorClusterState> temp = (ArrayList<MotorClusterState>) currentStatus.getActiveMotors();
//				MotorClusterState cluster = temp.get(0);
//				double time = currentStatus.getSimulationTime();
//				double result = cluster.getThrust(time);
//				System.out.println(currentStatus.getSimulationTime() + " " + result);
...
//				System.out.println(" time is " + currentStatus.getSimulationTime() + " current pressure is "  + currentStatus.getSimulationConditions().getAtmosphericModel().getConditions(currentStatus.getRocketWorldPosition().getAltitude()).getPressure()/100 + " the avg wind speed is " + ((PinkNoiseWindModel)currentStatus.getSimulationConditions().getWindModel()).getAverage());
//				System.out.println("simulation time is " + currentStatus.getSimulationTime());
				//End of eric code
...
//					System.out.println(currentStatus.getRocketPosition().y);
				}
...
//				}
			...
			// Note: setFlightData and setFlightDataBranch methods no longer exist in current API
			// The exception handling is done differently now
		...
		for (event = nextEvent(); event != null; event = nextEvent()) {
			...
			case RECOVERY_DEVICE_DEPLOYMENT:
				...
				if (currentStatus.getConfiguration().isStageActive(n)) {
					...
					//TODO ERRORMODE, this line does some wack shit so i commented it out, like it resets the sim??
...
//
	...
	private void checkNaN() throws SimulationException {
		...
		if (Double.isNaN(d) || b) {
			...
			throw new SimulationCalculationException(trans.get("BasicEventSimulationEngine.error.NaNResult"), currentStatus.getFlightDataBranch());
		}
	}
	...
	private FlightData computeCoastTime() throws SimulationException {
		try {
			...
			BasicEventSimulationEngineDispersionAnalysis e = new BasicEventSimulationEngineDispersionAnalysis();

			e.simulate(conds);
			return e.getFlightData();
		} catch (SimulationException e) {
			throw e;
		} catch (Exception e) {
