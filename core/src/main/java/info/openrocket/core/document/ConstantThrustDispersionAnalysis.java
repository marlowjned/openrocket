...
public class ConstantThrustDispersionAnalysis {
...
//    private double mainUpperBound = mainDeployTime + 3 ;
     ...
     */
    public ConstantThrustDispersionAnalysis(SimulationConditions presetConditions) {
...
//            mainDeployTime = mainDeploymentAltitudes[i];
            ...
            //Test variables
//            windDirection = Math.toRadians(0); //sets new value for wind direction
//            averageWindSpeed = 15;
//            deploymentTimeDrogue = 75.2;
//            mainDeployTime = 272;
...
//            setWindModel(); //re-active for normal wind models
        ...
        exportThrustData(); //after data is collected, it is exported
    }
...
//        mainDeploymentAltitudes =  distribution.uniformDistribution(mainLowerBound, mainUpperBound, iterations);
    ...
    public void configSimConditions() throws SimulationException {
        ...
        //ITERABLE PARAMETERS
//
//        conditions.setLaunchRodLength(launchRodLength); //   THIS SCREWS UP THE LANDING SPOT OF THE ROCKET
...
//                (recoveryDevices.get(1)).getDeploymentConfigurations().get(fcid).setDeployDelay(mainDeployTime);
    ...
    //Repeat of the prevous methods, used for thrust analysis only though
    public void saveThrustData() {
        //reset data variable

        //only care about dispersion at apogee, finds index of apogee
        double apogeeTime = flightSummary.getTimeToApogee();
        int apogeeIndex = 0;
        ArrayList<SimulationStatus> allflightData = ((BasicEventSimulationEngineDispersionAnalysis) simulator).getAllStatus();

        //finds index that apogee occurs
        for (int i = 0; i < allflightData.size(); i ++){
            if (allflightData.get(i).getSimulationTime() == apogeeTime){
                apogeeIndex = i;
                break;
            }
        }
        ...
        data[index][1]  = burnTime;
        data[index][2] = flightSummary.getMaxAltitude();
        data[index][3] = flightSummary.getFlightTime();
        data[index][4] = allflightData.get(apogeeIndex).getRocketPosition().x;
        data[index][5] = allflightData.get(apogeeIndex).getRocketPosition().y;
        data[index][6] = allflightData.get(apogeeIndex).getSimulationTime();
        ...
        PinkNoiseWindModel windModel = (PinkNoiseWindModel) allflightData.get(apogeeIndex).getSimulationConditions().getWindModel();
        data[index][9] = windModel.getAverage();
        data[index][10] = windModel.getDirection();
        data[index][11] = flightSummary.getLaunchRodVelocity();
    ...
    public void exportThrustData() {
        Object[] columnNames = {"number", "burntime", "Apogee", "flight time", "East position apogee", "North Position apogee",
                "Apogee time", "Final Position East", "Final Position North", "Average wind Speed", "Wind Speed Direction", "launch rod velocity"};
